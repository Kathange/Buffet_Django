...
# 更新新創建的資料庫讓其可以顯示在admin網頁中，需要在cmd中照順序打入下面兩行
...
class provide_food(models.Model):
    # name = models.CharField(max_length=100, primary_key=True)
    name = models.ForeignKey(food_code, primary_key=True, to_field='name', on_delete=models.RESTRICT, related_name='fk_prvideFood_name', db_column='name')
...
class food_info(models.Model):
    # food_id = models.IntegerField(primary_key=True)
    food_id = models.ForeignKey(food_code, primary_key=True, to_field='food_id', on_delete=models.RESTRICT, related_name='fk_foodInfo_foodId', db_column='food_id')
    weight = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    calorie = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    protein = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    carbohydrate = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    fat = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    cost = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    is_bone = models.BooleanField(null=False, blank=False)
    ...
    # def __str__(self):
    #     return f"{self.food_id}, {self.weight}, {self.calorie}, {self.protein}, {self.protein}, {self.carbohydrate}, {self.fat}, {self.cost}"


class user(models.Model):
    user_id = models.AutoField(primary_key=True)
    user_RFID = models.CharField(max_length=100, null=False, blank=False)
    user_line = models.CharField(max_length=100, null=False, blank=False, unique=True)
    # customer = models.BooleanField(default=True, null=False, blank=False)
    ...
    def __str__(self):
        return f"{self.user_RFID}, {self.user_line}"


class record(models.Model):
    record_id = models.AutoField(primary_key=True)
    # seller_id = models.ForeignKey(user, to_field='user_id', on_delete=models.RESTRICT, related_name="fk_record_sellerId")
    user_line = models.ForeignKey(user, to_field='user_line', on_delete=models.RESTRICT, related_name="fk_reocrd_userId", db_column='user_line')
    eat_date = models.DateField(null=False, blank=False)
    eat_time = models.TimeField(null=False, blank=False)
    food_img = models.CharField(max_length=100, null=True, blank=False)
    donut_chart_img = models.CharField(max_length=100, null=True, blank=False)
    sheet_img = models.CharField(max_length=100, null=True, blank=False)
    total_weight = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    total_cal = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    total_pro = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    total_carbo = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    total_fat = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
...
class detail_record(models.Model):
    ...
    record_id = models.ForeignKey(record, to_field='record_id', on_delete=models.RESTRICT, related_name='fk_detailRecord_recordId', db_column='record_id')
    food_id = models.ForeignKey(food_code, primary_key=True, to_field='food_id', on_delete=models.RESTRICT, related_name='fk_detailRecord_foodId', db_column='food_id')
    weight = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    calorie = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    protein = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    carbohydrate = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    ...
    # def __str__(self):
    #     return f"{self.record_id}, {self.food_id}, {self.weight}, {self.calorie}, {self.protein}, {self.carbohydrate}, {self.fat}"
    
    class Meta:
        managed = False
        # db_table: 'detail_record'
        unique_together = ('food_id', 'record_id')
        

class record_diff(models.Model):
    record_id = models.AutoField(primary_key=True)
    # seller_id = models.ForeignKey(user, to_field='user_id', on_delete=models.RESTRICT, related_name="fk_record_sellerId")
    user_line = models.ForeignKey(user, to_field='user_line', on_delete=models.RESTRICT, related_name="fk_reocrd_userId_diff", db_column='user_line')
    total_weight = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    total_cal = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    total_pro = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    total_carbo = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
...
class detail_record_diff(models.Model):
    # id = models.AutoField(primary_key=True)  # Add this line
    record_id = models.ForeignKey(record_diff, to_field='record_id', on_delete=models.RESTRICT, related_name='fk_detailRecord_recordId_diff', db_column='record_id')
    food_id = models.ForeignKey(food_code, primary_key=True, to_field='food_id', on_delete=models.RESTRICT, related_name='fk_detailRecord_foodId_diff', db_column='food_id')
    weight = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    calorie = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    protein = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    carbohydrate = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
...
class record_after(models.Model):
    record_id = models.AutoField(primary_key=True)
    # seller_id = models.ForeignKey(user, to_field='user_id', on_delete=models.RESTRICT, related_name="fk_record_sellerId")
    user_line = models.ForeignKey(user, to_field='user_line', on_delete=models.RESTRICT, related_name="fk_reocrd_userId_after", db_column='user_line')
    eat_date = models.DateField(null=False, blank=False)
    eat_time = models.TimeField(null=False, blank=False)
    donut_chart_img = models.CharField(max_length=100, null=True, blank=False)
    sheet_img = models.CharField(max_length=100, null=True, blank=False)
...
class detail_record_after(models.Model):
    # id = models.AutoField(primary_key=True)  # Add this line
    record_id = models.ForeignKey(record_after, to_field='record_id', on_delete=models.RESTRICT, related_name='fk_detailRecord_recordId_after', db_column='record_id')
    food_id = models.ForeignKey(food_code, primary_key=True, to_field='food_id', on_delete=models.RESTRICT, related_name='fk_detailRecord_foodId_after', db_column='food_id')
    weight = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    calorie = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    protein = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
    carbohydrate = models.DecimalField(max_digits=7, decimal_places=3, null=False, blank=False)
