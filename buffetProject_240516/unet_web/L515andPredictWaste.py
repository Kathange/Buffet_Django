...
if __name__ == "__main__":
    try:
        ...
        # Parse the JSON input
        try:
            input_data = json.loads(fInfo)
        except json.JSONDecodeError as e:
            print(f"Error decoding JSON: {e}")
            sys.exit(1)

        # Extract the two parts from the input data
        input_list = input_data.get("finfo")
        food_names_list = input_data.get("bone_food_names")

        # Process the 2D list and the bone food names list
        print("Received finfo in the subprocess:")
        print(input_list)
        print(type(input_list))  # Should be a list

        print("Received bone_food_names in the subprocess:")
        print(food_names_list)
        print(type(food_names_list))  # Should be a list
        ...
        if 'bone' in order:
            # 获取字符串的位置
            boneIndex = name_classes.index('bone') - 1
        # print("index", boneIndex)
        bonedepth = volume[boneIndex]
        bonedepth = bonedepth / len(food_names_list)
        print("bonedepth", bonedepth)
