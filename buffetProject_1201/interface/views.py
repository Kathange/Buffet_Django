...
detailed = [[0 for j in range(5)] for i in range(6)]
...
def unetPredict(request):
    ...
    try:
        ...
        show_to_cashier = volume_cla
        print(show_to_cashier)
        # Save merged data to a text file
        with open('volume.txt', 'w') as file:
            for item in show_to_cashier:
        ...
        # Remove the square brackets and then parse the string as a literal
        nutri = ast.literal_eval(nutristr)
        # Ensure it's a list of floats
        nutri = [[float(item) for item in sublist] for sublist in nutri]
        print("nutri~~: ", nutri)
        detailed = nutri
        print(detailed)
        # Save merged data to a text file
...
def cashier(request):
    ...
    img = qrcode.make('http://127.0.0.1:8000/interface/seller_info')
    ...
    context = {'cost': int(round(show_to_cashier[0], 0)), 'weight': round(show_to_cashier[1], 4), 'calories': round(show_to_cashier[2], 4)}
