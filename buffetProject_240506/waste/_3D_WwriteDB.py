...
def WwriteDB():     
    ...
    # 寫入 record_after 資料庫
    after_weight = (float(record_ori.total_weight) - total[0]) if (float(record_ori.total_weight) - total[0]) > 0 else 0
    after_cal = (float(record_ori.total_cal)-total[1]) if (float(record_ori.total_cal)-total[1]) > 0 else 0
    after_pro = (float(record_ori.total_pro)-total[2]) if (float(record_ori.total_pro)-total[2]) > 0 else 0
    after_carbo = (float(record_ori.total_carbo)-total[3]) if (float(record_ori.total_carbo)-total[3]) > 0 else 0
    after_fat = (float(record_ori.total_fat)-total[4]) if (float(record_ori.total_fat)-total[4]) > 0 else 0
    after_cost = (float(record_ori.cost)-cost) if (float(record_ori.cost)-cost) > 0 else 0
    ...
    flask_url = "https://a967-211-72-73-205.ngrok-free.app/newWaste"
    ...
    for item in show:
        ...
        if not detail_record_diff.objects.filter(food_id=food_object, record_id=recordunit_diff).exists():
            ...
            elif DRunit_diff.food_id in food_ids and not detail_record_after.objects.filter(food_id=food_object, record_id=recordunit_after).exists():
